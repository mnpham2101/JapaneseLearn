...
- [ ] 2.1.1 **[slint-developer]** In `ui/pages/study_page.slint`, add a stack-creation form that appears when the existing "Create Stack" button is clicked (use the "Vertically stacked up components pattern" from `slint-code-style.md`). The form contains a single `TextInput` for the stack name and Confirm/Cancel `CommonBtn` buttons. Add `callback stack-create-confirmed(name: string)` to `StudyPage`. No Rust logic yet — verify the form shows and hides correctly.
- [ ] 2.1.2 **[rust-developer]** In `src/main.rs`, wire `on_stack_create_confirmed`: create a new `FlashcardStackModel { stackname: name, flashcards: vec![] }`, append it to the current `flashcardList` using `VecModel`, and push it back to `StudyPage` via `set_flashcard_list`. **Depends on 2.1.1.**
- [ ] 2.2.1 **[slint-developer]** In `ui/components/flashcard_stack.slint`, add an "Add Flashcard" form below the card list (use the "Vertically stacked up components pattern"). The form contains two `TextInput` fields (Japanese word, Vietnamese meaning) and Confirm/Cancel `CommonBtn` buttons. Add `callback flashcard-add-confirmed(jap: string, meaning: string)` to `FlashcardStack`. Verify the form shows and hides correctly. **Independent of 2.1.1 — may run in parallel.**
- [ ] 2.2.2 **[rust-developer]** Wire `on_flashcard_add_confirmed`: create a new `FlashcardModel`, append it to the selected stack's `flashcards` using `VecModel`, and push the updated `flashcardList` back to `StudyPage`. **Depends on 2.2.1 and 2.1.2.**

  > Tasks 2.3.1 and 2.4.1 both modify `flashcard_stack.slint` — implement them sequentially.

- [ ] 2.3.1 **[slint-developer]** In `ui/components/flashcard_stack.slint`, wire the existing `TextInput` fields for editing: add `edited` callbacks on both `jap-obj` and `explanation` fields. Add `callback flashcard-field-changed(card-index: int, jap: string, meaning: string)` to `FlashcardStack`. The `TextInput` fields are already present — only callback declaration and wiring is needed. **Depends on 2.2.1.**
- [ ] 2.3.2 **[rust-developer]** Wire `on_flashcard_field_changed`: replace the `FlashcardModel` at `card-index` in the selected stack with updated field values, then push the refreshed `flashcardList` back to `StudyPage`. **Depends on 2.3.1.**
- [ ] 2.4.1 **[slint-developer]** In `ui/components/flashcard_stack.slint`, add a delete `CommonBtn` (e.g., "✕") per card row inside the `for card in` loop. Add `callback flashcard-delete-confirmed(card-index: int)` to `FlashcardStack`. **Depends on 2.3.1.**
- [ ] 2.4.2 **[rust-developer]** Wire `on_flashcard_delete_confirmed`: remove the `FlashcardModel` at `card-index` from the selected stack and push the updated `flashcardList` back to `StudyPage`. **Depends on 2.4.1.**
...
- [ ] 3.1 **[slint-developer]** In `ui/pages/study_page.slint`, add a study session view that activates when the user clicks a "Study" `CommonBtn` on the open `FlashcardStack`. The session displays one `Flashcard` component at a time (from task 1.3.3) centered on screen, with "Previous" and "Next" `CommonBtn` buttons and a `callback study-session-closed` to return to the stack list. Track `current-card-index` as an `in-out property <int>`. Use the existing `Flashcard` component — do not re-implement flip or known toggle. Apply the "Vertically stacked up components pattern" to manage visibility.

  > Tasks 3.2 and 3.3 both depend on 3.1 but are independent of each other — they may run in parallel.

- [ ] 3.2 **[slint-developer]** In the study session view, verify the reveal mechanism works end-to-end: the `Flashcard` component's `show-back` property (tap-to-flip, from task 1.3.3) shows the Japanese word first and hides the Vietnamese meaning until tapped. Confirm "Tap to reveal / Tap to hide" hint text updates correctly. No new Slint code is needed if `Flashcard` is wired correctly in 3.1 — this is a verification and any wiring fix task. **Depends on 3.1.**
- [ ] 3.3 **[slint-developer]** In the study session view, verify the known/unknown toggle works: the `Flashcard` component's `checkable: true; checked <=> known` binding (from task 1.3.4) handles marking in both directions. Task 3.4 ("toggle between known and unknown") is fully covered by this same mechanism — no separate implementation is needed. Confirm the toggle icon (✓/✗) and card background color update correctly. **Depends on 3.1.**

  > Tasks 3.5.1 and 3.5.2 are sequential.

- [ ] 3.5.1 **[slint-developer]** In `ui/pages/study_page.slint`, add a progress display to the study session view showing known vs unknown counts (e.g., "3 / 5 known"). Add `in property <int> known-count: 0` and `in property <int> total-count: 0` to the session component. Bind the display to these properties; use hardcoded placeholder values to verify the UI. **Depends on 3.3.**
- [ ] 3.5.2 **[rust-developer]** Wire live progress counts: compute `known-count` by iterating the active stack's `flashcards` and counting `known == true`; set `total-count` to the stack size. Recompute and push both values to `StudyPage` via weak handle whenever the known status changes (triggered by the existing `on_known_toggled` or a new `callback known-changed` from `Flashcard`). **Depends on 3.5.1.**
- [ ] 3.6 Test study mode interactions manually on Windows: flip cards, toggle known status, navigate prev/next, verify progress counts update in real time. **Depends on 3.5.2.**
