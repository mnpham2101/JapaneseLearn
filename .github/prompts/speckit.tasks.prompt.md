...
- [x] 6.7 **[slint-developer]** Vocabulary lesson list UI in `lib/vocabulary/ui/`.
  - [x] 6.7.1 Add lesson CRUD callbacks to `VocabularyAppLogic` — see [speckit.subtask.6-7-1.prompt.md](.github/prompts/speckit.subtask.6-7-1.prompt.md)
  - [x] 6.7.2 Implement `LessonList` component — see [speckit.subtask.6-7-2.prompt.md](.github/prompts/speckit.subtask.6-7-2.prompt.md)

  **Depends on 6.6.**
- [x] 6.8 **[slint-developer]** Vocabulary word form UI in `lib/vocabulary/ui/`.
  - [x] 6.8.1 Word form: spelling, kanji, meaning, type fields — see [speckit.subtask.6-8-1.prompt.md](.github/prompts/speckit.subtask.6-8-1.prompt.md)
  - [x] 6.8.2 Word form: tense list and example list — see [speckit.subtask.6-8-2.prompt.md](.github/prompts/speckit.subtask.6-8-2.prompt.md)

  **Depends on 6.7.**
- [x] 6.9 **[rust-developer]** `lib/vocabulary` Rust backend: persistence, CRUD handlers, vocabulary markdown import/export.
  - [x] 6.9.1 Persistence: `load_vocabulary()` / `save_vocabulary()` for `vocabulary.json` — see [speckit.subtask.6-9-1.prompt.md](.github/prompts/speckit.subtask.6-9-1.prompt.md)
  - [x] 6.9.2 CRUD handlers for lessons and words wired in `init()` — see [speckit.subtask.6-9-2.prompt.md](.github/prompts/speckit.subtask.6-9-2.prompt.md)
  - [x] 6.9.3 Vocabulary markdown import/export (`vocabulary_markdown_io.rs`) — see [speckit.subtask.6-9-3.prompt.md](.github/prompts/speckit.subtask.6-9-3.prompt.md)
...
- [x] 6.S.4 **[slint-developer + rust-developer]** Kanji flashcard large-font rendering — add `is-kanji` field and calligraphic styling.
  - [x] 6.S.4.1 **[slint-developer]** Add `is-kanji: bool` to `FlashcardModel` Slint struct; add `font-size-kanji: 72px` to `tokens.slint`; render front face with extra-large MS Mincho font when `is-kanji` is true. **Depends on 6.S.3.** — see [speckit.subtask.6-S-4-1.prompt.md](.github/prompts/speckit.subtask.6-S-4-1.prompt.md)
...
- [x] 6.B.1 **[slint-developer]** Fix `MatchingExerciseView` grid: cap front/back tile widths so both columns fit the viewport, and paginate to ~10 card pairs per page with a "Next" button. — see [speckit.subtask.6-B-1.prompt.md](.github/prompts/speckit.subtask.6-B-1.prompt.md)
...
- [x] 6.B.2 **[slint-developer + rust-developer]** Generate Flashcards notification — name newly created stacks and switch to the Flashcard tab.
  - [x] 6.B.2.1 **[slint-developer]** Add `generation-notification` and `active-view` properties to `VocabularyAppLogic`; bind `VocabularyPage` tabs to `active-view` and add a dismissible notification banner. — see [speckit.subtask.6-B-2-1.prompt.md](.github/prompts/speckit.subtask.6-B-2-1.prompt.md)
  - [x] 6.B.2.2 **[rust-developer]** In `on_generate_exercises_clicked`, diff pre-existing vs. generated stack names, set `generation_notification` naming only the new stacks, and set `active_view = 2`. **Depends on 6.B.2.1.** — see [speckit.subtask.6-B-2-2.prompt.md](.github/prompts/speckit.subtask.6-B-2-2.prompt.md)
- [x] 6.B.2.T **[slint-tester]** Test Generate Flashcards notification and tab switch.
  ...
  - Covers: Task 6.B.2
  **Depends on 6.B.2.2.**
- [x] 6.B.3 **[slint-developer]** Fix `FlashcardLabel` width overlap: cap width with `min(parent.width, ...)` so it no longer overflows its container border. — see [speckit.subtask.6-B-3.prompt.md](.github/prompts/speckit.subtask.6-B-3.prompt.md)
