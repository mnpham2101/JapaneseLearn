...
- [ ] 1.4.3 Implement `FlashcardStack` component in Slint that use a flashcard stack data model created in 1.4.1. The `FlashcardStack` component must display 2 text boxes for each flash card. It must provide a button to add new flashcards to the stack. Integrate this component with the `FlashcardLabel` component created in 1.4.2, so that clicking on a flashcard label opens the corresponding `FlashcardStack`.
