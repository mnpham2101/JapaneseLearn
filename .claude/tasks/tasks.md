...
- [ ] 8.V.7 **[rust-developer]** Rewrite example parsing in `lib/vocabulary/src/vocabulary_markdown_io.rs` to the `example: <sentence> : <meaning>` format (split on the first colon after the `example:` key, same second-colon pattern as tense), producing `ExampleData { sentence, meaning }` instead of a plain string; drop an example whose sentence is empty after trimming. Update existing example-related tests and add one reproducing the literal bundled-data pattern. **Depends on 8.V.6.**
- [ ] 8.V.8 **[slint-developer]** Add `in-out property <[FlashcardStackModel]> sentence-stack-list: []` and `in-out property <int> selected-sentence-stack-index: -1` to `FlashcardAppLogic` (`lib/flashcard/ui/flashcard_app_logic.slint`). Declaration only.
- [ ] 8.V.9 **[slint-developer]** `FlashcardManagerView` (`lib/flashcard/ui/page/flashcard_manager_view.slint`): add a "Flashcards"/"Sentences" toggle (two `CommonBtn`s) controlling which list/detail is shown; the Sentences view reuses `CommonList`/`FlashcardList`/`FlashcardStack` bound to `sentence-stack-list`/`selected-sentence-stack-index` (study-clicked and close-clicked wired the same way; the mutation callbacks — add/edit/delete/reorder card, delete stack — are intentionally left unwired for the Sentences view, since sentence stacks are machine-generated and fully replaced on every "Generate Exercises" run). **Depends on 8.V.8.**
- [ ] 8.V.10 **[rust-developer]** Add `sentences.json` persistence to `lib/flashcard/src/lib.rs`: `SENTENCES_FILE` const, `save_sentences`/`load_sentences` mirroring `save_stacks`/`load_stacks`, and `pub fn save_sentence_list<T>(ui: &T)` mirroring `save_current_list`. Call `load_sentences()` at `init()` startup to populate `sentence_stack_list`. **Depends on 8.V.8.**
- [ ] 8.V.11 **[rust-developer]** In `lib/vocabulary/src/lib.rs`'s `on_generate_exercises_clicked`, additionally dispatch `ExerciseRequest::Sentence` against the same lesson list, convert the `ExerciseOutput::Sentence` stacks to `flashcard::flashcard::FlashcardStackModel` (same conversion shape as the Flashcard branch), call `flashcard_logic.set_sentence_stack_list(...)` and `flashcard::save_sentence_list(&ui)`. See the cross-library architecture plan `.claude/tasks/architecture/task-8-V-11.md` (authored by project-owner) before implementing. **Depends on 8.V.5, 8.V.6, 8.V.7, 8.V.10.**
- [ ] 8.V.12 Update bundled default `.md` (and matching `.json`) datasets in `lib/vocabulary/ui/data/` so every `example:` line carries a meaning in the `sentence : meaning` format (no agent — data authored directly by task-manager). **Depends on 8.V.7.**
- [ ] 8.V.13 **[slint-tester]** Test tense-derived and sentence flashcard generation end-to-end in `test/vocabulary/`: a lesson with a word that has tenses and examples → `invoke_generate_exercises_clicked()` → assert the resulting flashcard stack contains the base card plus one card per tense with a conjugation, and assert `sentence-stack-list` contains one card per example with `front == sentence`, `back == meaning`. **Depends on 8.V.11.**
- [ ] 8.V.14 Manual verification on Windows (no agent): author a word with two tenses and one example, click Generate Exercises, confirm `data/stacks.json` gains the tense-derived cards and `data/sentences.json` is created, and confirm the FlashcardManagerView Sentences toggle shows the new sentence stack. **Depends on 8.V.13.**
